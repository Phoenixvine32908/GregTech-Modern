...
## Obj DynamicRender model changes.
The steps for registering obj models for DynamicRender(s) have been changed. Below is an explanation of the new system.
The class `com.gregtechceu.gtceu.client.util.ModelUtils;` has been renamed to `com.gregtechceu.gtceu.client.util.ModelEventHelper;` 
as well as the `registerBakeEventListener` method being refactored.

The old syntax of: `ModelUtils.registerBakeEventListener(boolean, Consumer<ModelEvent.BakingCompleted>)`
needs to be changed to the new syntax: `ModelEventHelper.registerBakeEventListener(boolean, QuadrupleFunction<ResourceLocation, BakedModel, transform.Root, ModelBakery, BakedModel>)`

An example of this change is below.
